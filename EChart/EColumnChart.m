...
- (void)setDataSource:(id<EColumnChartDataSource>)dataSource
{
    if (_dataSource != dataSource)
    {
        ...
        
        {
            ...
            float highestValueEColumnChart = [_dataSource highestValueEColumnChart:self].value * HIGHEST_FACTOR;
            // zhenlintie 2014-04-15 取整数
//            int highestValueEColumnChartInt = (int)ceil(highestValueEColumnChart);
            
            int valueGap = (int)floor((highestValueEColumnChart / 10.0f));
            float heightGap = self.frame.size.height * (valueGap / highestValueEColumnChart);
            float padding = CGRectGetHeight(self.frame)-10*heightGap;
            for (int i = 0; i < 11; i++)
            {
                // 再加一条线
                if (0 == i){
                    UIView *horizontalLineTop = [[UIView alloc] initWithFrame:CGRectMake(0, heightGap * (i-1) + padding, self.frame.size.width, HORIZONTAL_LINE_HEIGHT)];
                    horizontalLineTop.backgroundColor = ELightGrey;
                    [self addSubview:horizontalLineTop];
                }
//                float valueGap = highestValueEColumnChart / 10.0;
                UIView *horizontalLine = [[UIView alloc] initWithFrame:CGRectMake(0, heightGap * i + padding, self.frame.size.width, HORIZONTAL_LINE_HEIGHT)];
                horizontalLine.backgroundColor = ELightGrey;
                NSLog(@"-- %d %@",i,[NSValue valueWithCGRect:horizontalLine.frame]);
                [self addSubview:horizontalLine];
                
                // 再加一条线
                if (0 == i){
                    EColumnChartLabel *eColumnChartLabelTop = [[EColumnChartLabel alloc] initWithFrame:CGRectMake(-1 * Y_COORDINATE_LABEL_WIDTH, -heightGap / 2.0 + heightGap * (i-1) + padding, Y_COORDINATE_LABEL_WIDTH, heightGap)];
                    [eColumnChartLabelTop setTextAlignment:NSTextAlignmentCenter];
                    eColumnChartLabelTop.text = [[NSString stringWithFormat:@"%d ",valueGap * (10 - (i-1))] stringByAppendingString:[_dataSource highestValueEColumnChart:self].unit?:@""];
                    
                    //eColumnChartLabel.backgroundColor = ELightBlue;
                    [self addSubview:eColumnChartLabelTop];
                }
                
                EColumnChartLabel *eColumnChartLabel = [[EColumnChartLabel alloc] initWithFrame:CGRectMake(-1 * Y_COORDINATE_LABEL_WIDTH, -heightGap / 2.0 + heightGap * i + padding, Y_COORDINATE_LABEL_WIDTH, heightGap)];
...
- (void)reloadData
{
    ...
    for (int i = 0; i < totalColumnsRequired; i++)
    {
        ...
        {
            eColumnChartLabel = [[EColumnChartLabel alloc] initWithFrame:CGRectMake(widthOfTheColumnShouldBe * 0.5 + (i * widthOfTheColumnShouldBe * 1.5), self.frame.size.height, widthOfTheColumnShouldBe, 70)];
            [eColumnChartLabel setTextAlignment:NSTextAlignmentCenter];
